...
class GameEngine {
   ...
   * @param {Object|number} options - { players: string[] } or playerCount number (2-4)
   * @returns {Object} Initial game state
   */
  static createGame(gameType, options = {}) {
    // Support passing playerCount as a number
    if (typeof options === 'number') {
      const count = options;
      // Assign opposite positions for competitive balance
      // Board layout: Red(top-left), Green(top-right), Yellow(bottom-right), Blue(bottom-left)
      // 2 players: Red vs Yellow (diagonal opposites)
      // 3 players: Red, Green, Yellow (maximizes spacing)
      // 4 players: all four
      const playerSets = {
        2: ['red', 'yellow'],
        3: ['red', 'green', 'yellow'],
      ...
      options = { players: playerSets[count] || playerSets[4] };
    }
